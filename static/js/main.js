...
async function diagFetch(url, opts = {}) {
  ...
  try {
    ...
    console.debug('[diagFetch] response:', {
      url,
      status: res.status,
      ok: res.ok,
      bodyPreview: Array.isArray(body) ? body.slice(0, 3) : body
    });
...
function buildCharts(logs) {
  ...
  if (ctx) {
    ...
    typeChart = new Chart(ctx, { type: 'pie', data: { labels: labels, datasets: [{ data: data }] } });
  }
...
(async function init() {
  ...
  setInterval(async () => {
    ...
    if (Math.random() < 0.12) try { await populateFilters(); } catch (e) { /* ignore */ }
  }, REFRESH_MS);
})();
